...
class cMenu:
    ...
    #  @param   self          The class itself, Python standard
    #
    #  Makes the selection of the menu's items based on mouse input
    #  This method was added by Andre Costa, doesn't belong to the original class
    #
    def update_mouse(self):
        pt = pygame.mouse.get_pos()
        for i in range(0, len(self.menu_items)):
            if self.menu_items[i]['offset'][0] + self.menu_items[i]['rect'].width > \
                    pt[0] > self.menu_items[i]['offset'][0] and \
                    self.menu_items[i]['offset'][1] + self.menu_items[i]['rect'].height > \
                    pt[1] > self.menu_items[i]['offset'][1]:
                self.selection = i
                self.menu_items[self.selection]['selected'] = True
                for j in range(0, len(self.menu_items)):
                    if j != i:
                        self.menu_items[j]['selected'] = False
        self.redraw_all()
